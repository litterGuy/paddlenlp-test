...
```python
# 样本构建：语句级情感分类任务
python label_studio.py  --label_studio_file data/label_studio.json  --task_type cls  --save_dir data  --splits 0.8 0.1 0.1  --
options "Positive" "Negative" "Neutral" --is_shuffle True  --seed 1000

# paddle-gpu 版本要和nvidia对应

# 模型训练 显存不足，调低batch_size
python -u -m paddle.distributed.launch --gpus "0" finetune.py  --train_path data/train.json --dev_path data/dev.json --save_dir checkpoint --learning_rate 1e-5 --batch_size 4 --max_seq_len 512 --num_epochs 3 --model uie-senta-base --seed 1000 --logging_steps 10 --valid_steps 100 --device gpu
```
